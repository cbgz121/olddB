...
func TestBuild(t *testing.T) {
	path := "/tmp/simpledb"
	_ = os.MkdirAll("/tmp/simpledb", os.ModePerm)
	_ = os.MkdirAll("/tmp/simpledb/"+mergeDir, os.ModePerm)
...
func TestNewDBFile(t *testing.T) {
	...
	tests := []struct {
		...
		{"f-string", args{"/tmp/simpledb", 0, FileIO, 1 * 1024 * 1024, String}, nil, false},
		{"f-list", args{"/tmp/simpledb", 0, FileIO, 1 * 1024 * 1024, List}, nil, false},
		{"f-hash", args{"/tmp/simpledb", 0, FileIO, 1 * 1024 * 1024, Hash}, nil, false},
		{"f-set", args{"/tmp/simpledb", 0, FileIO, 1 * 1024 * 1024, Set}, nil, false},
		{"f-zset", args{"/tmp/simpledb", 0, FileIO, 1 * 1024 * 1024, ZSet}, nil, false},

		{"m-string", args{"/tmp/simpledb", 1, MMap, 1 * 1024 * 1024, String}, nil, false},
		{"m-list", args{"/tmp/simpledb", 2, MMap, 2 * 1024 * 1024, List}, nil, false},
		{"m-hash", args{"/tmp/simpledb", 3, MMap, 3 * 1024 * 1024, Hash}, nil, false},
		{"m-set", args{"/tmp/simpledb", 4, MMap, 4 * 1024 * 1024, Set}, nil, false},
		{"m-zset", args{"/tmp/simpledb", 5, MMap, 5 * 1024 * 1024, ZSet}, nil, false},
	}
